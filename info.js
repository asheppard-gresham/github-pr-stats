...
const format_pr_short = async(req) => {
  ...
  return {
    ...
    'number': req.number,
    'created_at': start,
    'created_at_day': start.format('DD/MM/YYYY'),
    'updated_at': end,
...
const generateReport = async (daysToGoBack, pullRequests, averageOnly) => {
  ...
  for (const key in prs_by_day) {
    var table = new Table({
      head: ['Date'.white, 'Number'.white, 'Title'.white, 'Open For'.white],
      colWidths: [15, 10, 60, 30]
    });
    ...
    prs_by_day[key].map(async(pr) => {
      ...
      if (openedFor.days() >= 1) {
        table.push([key, pr.number, pr.title.substring(0, 50).red, timeTaken.red]);
      } else {
        table.push([key, pr.number, pr.title.substring(0, 50).green, timeTaken.green]);
      }
    });
...
const display_open_pull_requests = async(repo) => {
  ...
  var table = new Table({
    head: ['User'.white, 'Number'.white, 'Title'.white, 'Opened For'.white],
    colWidths: [20, 10, 60, 30]
  });
  ...
  for (const req in requests) {
    ...
    if (openedFor.days() >= 1) {
      table.push([pr.user.red, pr.number, pr.title.substring(0, 50).red, pr.opened_for.red]);
    } else {
      table.push([pr.user.green, pr.number, pr.title.green.substring(0, 50), pr.opened_for.green]);
    }
  }
